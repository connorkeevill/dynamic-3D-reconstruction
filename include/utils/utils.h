...
namespace refusion {
	...
	inline float norm(const float3 &vec)
	{
	...
	inline int sign(float n) { return (n > 0) - (n < 0); }
	...
	inline float3 ColorToFloat(uchar3 c)
	{
	...
	inline float3 GetPoint3d(int i, float depth, RgbdSensor sensor)
	{
		int v = i / sensor.cols;
	...
	inline int2 Project(float3 point3d, RgbdSensor sensor)
	{
		float2 point2df;
