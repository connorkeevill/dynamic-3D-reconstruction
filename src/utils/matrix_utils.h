...
//////////////////////////////
	...
	public:
		...
		float2x2 operator*(const float t) const
		{
			float2x2 res;
			res.m11 = m11 * t;
			res.m12 = m12 * t;
...
//////////////////////////////
	...
	public:
		...
		float2 operator*(const float3 &v) const
		{
		...
		float2x3 operator*(const float t) const
		{
			float2x3 res;
			res.m11 = m11 * t;
			res.m12 = m12 * t;
...
//////////////////////////////
	...
	public:
		...
		float3 operator*(const float2 &v) const
		{
		...
		float3x2 operator*(const float t) const
		{
			float3x2 res;
			res.m11 = m11 * t;
			res.m12 = m12 * t;
	...
	float2x2 matMul(const float2x3 &m0, const float3x2 &m1)
	{
		float2x2 res;
	...
	public:
		...
		explicit inline __device__ __host__

		float3x3(const float2x2 &other)
		{
			m11 = other.m11;
			m12 = other.m12;
			m13 = 0.0;
		...
		void setRow(unsigned int i, float3 &r)
		{
			entries[3 * i + 0] = r.x;
			entries[3 * i + 1] = r.y;
			entries[3 * i + 2] = r.z;
		...
		void mult(const float3x3 &other)
		{
			float3x3 res;
		...
		float3x3 operator*(const float t) const
		{
			float3x3 res;
			res.m11 = m11 * t;
			res.m12 = m12 * t;
		...
		float3x3 getDiagonalMatrix(float diag = 1.0f)
		{
			float3x3 res;
			res.m11 = diag;
			res.m12 = 0.0f;
		...
		float3x3 tensorProduct(const float3 &v, const float3 &vt)
		{
			float3x3 res;
			res.m11 = v.x * vt.x;
			res.m12 = v.x * vt.y;
	...
	float2x3 matMul(const float2x3 &m0, const float3x3 &m1)
	{
		float2x3 res;
		res.m11 = m0.m11 * m1.m11 + m0.m12 * m1.m21 + m0.m13 * m1.m31;
		res.m12 = m0.m11 * m1.m12 + m0.m12 * m1.m22 + m0.m13 * m1.m32;
...
// (1x2) row matrix as float2
	...
	float3 matMul(const float2 &m0, const float2x3 &m1)
	{
		float3 res;
	...
	public:
		...
		float3x4(const float values[12])
		{
			m11 = values[0];
			m12 = values[1];
			m13 = values[2];
		...
		float3x4 operator=(const float3x4 &other)
		{
			m11 = other.m11;
			m12 = other.m12;
			m13 = other.m13;
		...
		float3x4 &operator=(const float3x3 &other)
		{
			m11 = other.m11;
			m12 = other.m12;
			m13 = other.m13;
		...
		float3x4 operator*(const float t) const
		{
			float3x4 res;
			res.m11 = m11 * t;
			res.m12 = m12 * t;
		...
		float3x4 &operator*=(const float t)
		{
			*this = *this * t;
			return *this;
		}
		...
		float3x4 operator*(const float3x3 &other) const
		{
			float3x4 res;
			res.m11 = m11 * other.m11 + m12 * other.m21 + m13 * other.m31;
			res.m12 = m11 * other.m12 + m12 * other.m22 + m13 * other.m32;
		...
		void setFloat3x3(const float3x3 &other)
		{
			m11 = other.m11;
			m12 = other.m12;
			m13 = other.m13;
	...
	public:
		...
		float4x4(const float values[16])
		{
			m11 = values[0];
			m12 = values[1];
			m13 = values[2];
		...
		float4x4 operator=(const float3x4 &other)
		{
			m11 = other.m11;
			m12 = other.m12;
			m13 = other.m13;
		...
		float4x4 &operator=(const float3x3 &other)
		{
			m11 = other.m11;
			m12 = other.m12;
			m13 = other.m13;
		...
		void setFloat3x4(const float3x4 &other)
		{
			m11 = other.m11;
			m12 = other.m12;
			m13 = other.m13;
...
//////////////////////////////
	...
	public:
		...
		matNxM<N, M> &operator=(const matNxM<N, M> &other)
		{
			__CONDITIONAL_UNROLL__
		...
		matNxM(const B &other);

		template<class B>
		explicit inline __device__ __host__

		matNxM(const B &other0, const B &other1);
		...
		matNxM<N, MOther> operator*(const matNxM<NOther, MOther> &other) const
		{
			cudaAssert(M == NOther);
			matNxM<N, MOther> res;
		...
		matNxM<N, M> operator+(const matNxM<N, M> &other) const
		{
			matNxM<N, M> res = (*this);
			res += other;
			return res;
		...
		matNxM<N, M> operator*(const float t) const
		{
			matNxM<N, M> res = (*this);
			res *= t;
			return res;
		...
		matNxM<N, M> &operator*=(const float t)
		{
			__CONDITIONAL_UNROLL__
		...
		void getBlock(unsigned int xStart, unsigned int yStart, matNxM<NOther, MOther> &res) const
		{
		...
		void setBlock(matNxM<NOther, MOther> &input, unsigned int xStart, unsigned int yStart)
		{
			cudaAssert(xStart + NOther <= N && yStart + MOther <= M);
...
//////////////////////////////
	...
	matNxM<N, M> operator*(const float t, const matNxM<N, M> &mat)
	{
		matNxM<N, M> res = mat;
		res *= t;
		return res;
...
//////////////////////////////
	...
	float matNxM<3, 3>::det() const
	{
		...
		return m11 * m22 * m33 + m12 * m23 * m31 + m13 * m21 * m32 - m31 * m22 * m13 - m32 * m23 * m11 -
			   m33 * m21 * m12;
	}
	...
	matNxM<3, 3> matNxM<3, 3>::getInverse() const
	{
		matNxM<3, 3> res;
		res.entries[0] = entries[4] * entries[8] - entries[5] * entries[7];
		res.entries[1] = -entries[1] * entries[8] + entries[2] * entries[7];
	...
	matNxM<2, 2> matNxM<2, 2>::getInverse() const
	{
		matNxM<2, 2> res;
		res(0, 0) = (*this)(1, 1);
		res(0, 1) = -(*this)(0, 1);
...
// To Matrix from floatNxN
	...
	matNxM<1, 1>::matNxM(const float &other)
	{
		entries[0] = other;
	}
...
// To Matrix from floatNxN
	...
	matNxM<2, 2>::matNxM(const float2x2 &other)
	{
		__CONDITIONAL_UNROLL__
	...
	matNxM<3, 3>::matNxM(const float3x3 &other)
	{
		__CONDITIONAL_UNROLL__
	...
	matNxM<4, 4>::matNxM(const float4x4 &other)
	{
		__CONDITIONAL_UNROLL__
	...
	matNxM<3, 2>::matNxM(const float3 &col0, const float3 &col1)
	{
...
// To Matrix from floatN
	...
	matNxM<2, 1>::matNxM(const float2 &other)
	{
		entries[0] = other.x;
		entries[1] = other.y;
	}
	...
	matNxM<3, 1>::matNxM(const float3 &other)
	{
		entries[0] = other.x;
		entries[1] = other.y;
		entries[2] = other.z;
	...
	matNxM<4, 1>::matNxM(const float4 &other)
	{
		entries[0] = other.x;
		entries[1] = other.y;
		entries[2] = other.z;
